...
lm = scr_w/2 - 1*scr_w/2+win.margin(1)*scr_w/100;
mm = scr_w/2 + 0*scr_w/scale;
mr = scr_w/2 + 1*scr_w/2-win.margin(1)*scr_w/100;
% rr = scr_w/2 + 2*scr_w/scale;
% % top, top-center, center, center-bottom, bottom
% tt = scr_h/2 - 2*scr_h/scale;
tc = scr_h/2 - 1*scr_h/2+win.margin(2)*scr_h/100;
cc = scr_h/2 + 0*scr_h/scale;
cb = scr_h/2 + 1*scr_h/2-win.margin(2)*scr_h/100;
...
cmds(end+1) = EyelinkCmd( ...
                ['calibration_targets = %d,%d %d,%d %d,%d %d,%d %d,%d'...
                 ' %d,%d %d,%d %d,%d %d,%d'], ...
                 mm,cc, mm,tc, mm,cb, lm,cc, mr,cc, lm,tc, mr,tc, lm,cb, mr,cb);
